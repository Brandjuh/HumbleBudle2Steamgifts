...
(function (root) {
  ...
  /**
   * Keert Humble's verbodslijst om naar de toestemmingslijst die SteamGifts wil.
   *
   * Humble levert `disallowed_countries` — landen waar de key níét werkt.
   * SteamGifts' regio-restrictie werkt andersom: je vinkt aan wie wél mee mag
   * doen. Zonder deze omkering zou een giveaway open staan voor mensen die de
   * key niet kunnen activeren.
   *
   * Landcodes die SteamGifts niet kent worden simpelweg genegeerd; codes die
   * Humble niet noemt blijven toegestaan.
   *
   * @param {string[]} availableIds landcodes zoals SteamGifts ze aanbiedt
   * @param {string[]} disallowed landcodes uit Humble
   * @returns {{allowed: string[], blocked: string[], unknown: string[]}}
   */
  HSG.allowedCountries = function (availableIds, disallowed) {
    const available = (availableIds || []).map((id) => String(id).trim()).filter(Boolean);
    const blockSet = new Set(
      (disallowed || []).map((code) => String(code).trim().toUpperCase()).filter(Boolean)
    );
    const availableSet = new Set(available.map((id) => id.toUpperCase()));

    const allowed = available.filter((id) => !blockSet.has(id.toUpperCase()));
    const blocked = available.filter((id) => blockSet.has(id.toUpperCase()));
    const unknown = Array.from(blockSet).filter((code) => !availableSet.has(code));
