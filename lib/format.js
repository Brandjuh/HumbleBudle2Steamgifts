...
(function (root) {
   ...
   * SteamGifts vraagt het omgekeerde: welke landen mogen meedoen. Zonder deze
   * omkering staat een giveaway open voor mensen die de key niet kunnen
   * activeren.
   *
   * @param {{id: string, code: string|null}[]} items landrijen van SteamGifts
   * @param {string[]} disallowed ISO-landcodes uit Humble
   * @returns {{allowedIds: string[], blockedCodes: string[], unknownCodes: string[], unmapped: string[]}}
   */
  HSG.allowedCountries = function (items, disallowed) {
    ...
    for (const item of items || []) {
      ...
      if (!code) {
        // Geen leesbare landcode. Humble noemt dit land dan per definitie niet,
        // dus toestaan — maar wel melden, want het wijst op een opmaakwijziging.
        unmapped.push(item.id);
        allowedIds.push(item.id);
        continue;
      }
      known.add(code);
      if (blockSet.has(code)) blockedCodes.push(code);
      else allowedIds.push(item.id);
    }

    const unknownCodes = Array.from(blockSet).filter((code) => !known.has(code));
